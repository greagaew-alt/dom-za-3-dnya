...
def page_bani():
  ...
  <div class="split">
    ...
    b.append(sec("Размеры и цены бань", """
  ...
  %s""" % table(["Размер", "Состав", "Срок", "Цена"], [
        ...
        "сравнить размер и цену."))

    b.append(sec("Что входит в баню", """
  ...
  <p class="small" style="margin-top:11px">Отдельно считаются: %s</p>""" % (
        ...
        "Здесь тот же принцип, что в комплектациях домов: явно разделяем "
        "«входит» и «считается отдельно», чтобы не было сюрпризов."))

    b.append(sec("Заявка на баню", """
  <div class="split">
    <div>
      <h2>Посчитаем баню за минуту</h2>
      <p class="lead">Назовите размер и что нужно внутри &mdash; скажем цену сразу
      в разговоре.</p>
    </div>
    <div>%s</div>
  </div>""" % form("Оставьте заявку", ["Ваше имя", "Телефон"], "Узнать цену"),
        "Точка захвата на странице бань."))
